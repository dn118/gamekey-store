...
async function createOrder({ token, orderId, promoCode }) {
  return request("/api/orders", { method: "POST", headers: jsonHeaders, body: JSON.stringify({ client_token: token, order_id: orderId, sku: "STEAM-TOPUP-500", promo_code: promoCode }) });
}
...
const diagnostics = await request(`/api/orders/${orderId}`);
